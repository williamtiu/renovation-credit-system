...
Abstract
Introduction and background (Laurance 林)
Market research (Laurance 林)
Business model (Laurance 林)
Key algo/model (Laurance 林)/ (@成友)
System Architecture (@成友)
Workflow (@哥斯拉🦕)
UI/UX design (@哥斯拉🦕)
Backend (@成友)
database (@Tracy)
Software QA Test  (@Tracy)
It security and compliance (William Tiu)
Limitations & Future Work (William Tiu)
Team and workduiabltion (Team and work distribution) (William Tiu)

---
## Abstract
DecoFinance represents a paradigm-shifting, multi-role B2B2C FinTech ecosystem engineered to resolve the deeply ingrained systemic trust deficits and persistent financing gaps crippling the Hong Kong renovation and interior design market. For decades, the local renovation industry has been characterized by profound information asymmetry, archaic contracting norms, and a complete absence of standardized, institutionalized financial protections. Consumers are routinely subjected to exorbitant upfront capital demands—often ranging from 30% to 40% of the total project valuation—exposing them to the catastrophic financial risk colloquially known as "abandoned projects" (爛尾裝修), where contractors misallocate funds or enter insolvency prior to project completion. Conversely, Small-to-Medium Enterprise (SME) renovation contractors endure suffocating cash-flow bottlenecks. Tasked with procuring raw materials and dispatching labor prior to subsequent milestone payouts, these SMEs require bridging capital. However, traditional financial institutions (commercial banks) systematically reject SME loan applications due to the inherent volatility of construction projects, the absence of physical collateral, and the SMEs' inability to provide multi-year, CPA-audited financial statements.
...
### 4. Revenue Streams
*   **Escrow Administration (1.5% - 2.5%):** Deducted from the Gross Contract Value (GCV) upon successful final milestone payment. This baseline fee mirrors enterprise payment gateways but provides exponentially higher intrinsic value (capital protection and conflict arbitration).
*   **Financing Margins (6% - 12% APR):** The paramount revenue driver. High-rated SMEs receive short-term bridging credit. Because the platform auto-liquidates loan principal via escrow splits at milestone completion, DecoFinance secures wholesale institutional backing at extremely low base rates (e.g., 3%), yielding a massive 3% to 9% net interest margin with near-zero downside risk.
*   **Premium Compliance SaaS:** Subscription model granting elite "Premium Verified" status, accelerating BD/OSH validation and boosting organic B2B lead generation.

### 5. Financial Projections & Risk Mitigation
To ensure viability, the model must address key startup constraints:
*   **Regulatory Risk (HKMA Cap 163):** Issuing loans in Hong Kong requires strict adherence to the Money Lenders Ordinance and PDPO guidelines. DecoFinance operates via licensed institution partnerships, packaging the API data payloads rather than acting as a direct unregulated lender.
*   **Collusion Risk Mitigation:** To prevent contractors and customers from colluding to extract loan capital, DecoFinance mandates robust multi-part photographic evidence uploads (`evidence_urls`) validated by internal Reviewer grids and immutable audits.
...
flowchart TD
    ...
    subgraph Frontend
        SSR[Jinja2 / Static HTML]
        SPA[React.js Vite UI - /new-ui]
    end
    
    subgraph Flask Backend
        Router[Flask Application / Blueprints]
        Auth[utils/auth_helper.py]
        
        subgraph Services
            Scorer[services/credit_scorer.py]
            Escrow[services/escrow_manager.py]
        end
    end
...
sequenceDiagram
    ...
    participant P as Platform (DecoFinance)
    participant S as SME Contractor
    participant B as Banking Partner

    Note over S, P: Stage 1: Identity & Compliance
    S->>P: Register & Upload Specs (BD/OSH)
    P-->>S: Baseline 4D Trust Score Assigned

    Note over C, P: Stage 2: Project Structuring
    C->>P: Create Project via Wizard (Budget, Style)
    P-->>S: Project Broadcasted to SMEs

    Note over C, S: Stage 3: Bidding & Escrow
    S->>P: Submit Bid & Design Diagram
    C->>P: Approves Bid
    C->>P: Deposit Full Funds to Escrow
    P-->>P: Smart Contract Locked (State: FUNDED)

    Note over S, B: Stage 4: Intelligent Financing
    S->>P: Request Material Loan against Escrow
    P->>B: Encrypted Borrower Data & Trust Score
    B-->>S: Loan Advanced (Secured by Escrow)

    Note over C, P: Stage 5: Milestone Execution & Release
    S->>P: Milestone Finished & Proof Uploaded
    P-->>C: Prompt for Review (Chase Progress)
    C->>P: Accept Milestone
    P->>P: Atomic Splitting (Deduct Loan Principal & Interest)
    P-->>S: Residual Funds Transferred to SME
    P-->>B: Loan Repayment Transferred
```
...
The foundational data layer leverages SQLite via SQLAlchemy ORM, mitigating SQL-injection and handling intense 1-to-Many relationships.

### Safe Relational Diagram
```mermaid
erDiagram
    USER ||--o| COMPANY : owns
    COMPANY ||--o{ PROJECT_BID : places
    USER ||--o{ PROJECT : creates
    PROJECT ||--|{ PROJECT_MILESTONE : contains
    PROJECT ||--o| SMART_CONTRACT : governed_by
    SMART_CONTRACT ||--o{ ESCROW_LEDGER : records_transactions
    COMPANY ||--o{ LOAN_APPLICATION : applies
```

`Projects` forms the macro root, bridging to `Bids` and `Milestones`. To aggressively improve read-ops and support dense media, massive parameter arrays (such as AWS `evidence_urls`) are stored utilizing SQLAlchemy native `JSON` columns explicitly attached to the primary relational keys, maintaining atomic conformity without unnecessary junction tables.
...
## Software QA Test (@Tracy)
Given the transactional stakes involved in the Escrow and the algorithm-determined loan structures, assuring software execution stability across extreme scenarios is mission-critical. DecoFinance heavily integrates automated assurance environments:
*   **Unit ;& Algorithmic Testing (Pytest):** Explicit test suites assert the core mathematical logic driving `services/credit_scorer.py`. Mathematical mocks guarantee that boundary parameters (e.g., verifying exactly `10,000,000 HKD` registered capital equates to `150` internal points) never drift following future codebase regressions.
*   **UI/UX Verification (Selenium):** Comprehensive end-to-end (E2E) testing targets the React/Vite interfaces. Specifically targeting complex component structures encompassing the B2C Project Structuring Wizard and B2B Asynchronous photo upload handlers, ensuring network timeouts gracefully rebound without permanently locking an active Escrow state.
*   **Compliance to ISO/IEC 25010 Standards:** This validation philosophy directly addresses core criteria for Reliability and Functional Suitability, assuring strict state alignment within the `EscrowLedgerEntry` limits during multi-user concurrency.
