...
def _apply_company_form(company):
    ...
    company.company_registration_number = request.form.get(
        'company_registration_number') or None
    company.established_date = _parse_date_value(
        request.form.get('established_date'))
    company.registered_capital = _parse_float_value(
        request.form.get('registered_capital'), 0.0) or 0.0
    ...
    company.employee_count = _parse_int_value(
        request.form.get('employee_count'), 0) or 0
    company.annual_revenue = _parse_float_value(
        request.form.get('annual_revenue'), 0.0) or 0.0
    company.current_assets = _parse_float_value(
        request.form.get('current_assets'), 0.0) or 0.0
    company.current_liabilities = _parse_float_value(
        request.form.get('current_liabilities'), 0.0) or 0.0
    company.total_cash = _parse_float_value(
        request.form.get('total_cash'), 0.0) or 0.0
    company.total_liabilities = _parse_float_value(
        request.form.get('total_liabilities'), 0.0) or 0.0
    company.shareholders_equity = _parse_float_value(
        request.form.get('shareholders_equity'), 0.0) or 0.0
    ...
    company.manual_current_ratio = _parse_optional_float(
        request.form.get('manual_current_ratio'))
    company.manual_cash_ratio = _parse_optional_float(
        request.form.get('manual_cash_ratio'))
    company.manual_debt_to_equity_ratio = _parse_optional_float(
        request.form.get('manual_debt_to_equity_ratio'))

    company.project_count_completed = _parse_int_value(
        request.form.get('project_count_completed'), 0) or 0
    company.average_project_value = _parse_float_value(
        request.form.get('average_project_value'), 0.0) or 0.0
    ...
    company.licence_expiry_date = _parse_date_value(
        request.form.get('licence_expiry_date'))
    company.licence_verification_status = request.form.get(
        'licence_verification_status', 'pending')
    company.osh_officer_role = request.form.get('osh_officer_role') or None
    company.osh_officer_license_number = request.form.get(
        'osh_officer_license_number') or None
    company.insurance_provider = request.form.get('insurance_provider') or None
    company.insurance_policy_number = request.form.get(
        'insurance_policy_number') or None
    company.insurance_expiry_date = _parse_date_value(
        request.form.get('insurance_expiry_date'))
    company.insurance_verification_status = request.form.get(
        'insurance_verification_status', 'pending')
    company.safety_training_coverage = _clamp_percentage(
        _parse_int_value(request.form.get('safety_training_coverage')))
    company.safety_incident_count = _parse_int_value(
        request.form.get('safety_incident_count'), 0) or 0
    company.esg_policy_level = (request.form.get(
        'esg_policy_level') or 'none').lower()
    company.green_material_ratio = _clamp_percentage(
        _parse_int_value(request.form.get('green_material_ratio')))


def _build_monitoring_alerts(company, latest_score, overdue_accounts, open_disputes):
    alerts = _parse_risk_factors(
        latest_score.risk_factors if latest_score else None)
    ...
    if overdue_accounts > 0:
        alerts.append(
            f'{overdue_accounts} overdue credit account(s) need review')

    if open_disputes:
        alerts.append(
            f'{len(open_disputes)} open dispute case(s) remain unresolved')
...
def _build_audit_snapshot(company, loan_applications, disputes):
    ...
    company_logs = AuditLog.query.filter_by(
        target_type='Company', target_id=company.id).order_by(AuditLog.created_at.desc()).all()
    ...
    score_log = next(
        (log for log in company_logs if log.action == 'trust_score_calculated'), None)
    verification_log = next((log for log in company_logs if log.action in [
                            'company_updated', 'company_created']), None)
    loan_decision_log = next((log for log in loan_logs if log.action in [
                             'loan_application_approved', 'loan_application_rejected', 'loan_disbursed']), None)
    dispute_log = next((log for log in dispute_logs if log.action in [
                       'dispute_resolved', 'dispute_opened']), None)

    combined_logs = sorted(
        company_logs[:8] + loan_logs[:8] + dispute_logs[:8],
        key=lambda log: log.created_at or datetime.min.replace(
            tzinfo=timezone.utc),
        reverse=True,
    )[:10]
...
def _load_latest_score_map(companies):
    ...
    for company in companies:
        latest_scores[company.id] = CreditScore.query.filter_by(
            company_id=company.id).order_by(CreditScore.scored_at.desc()).first()
...
def _build_credit_report(company, latest_score, credit_scores, loan_applications, project_bids, disputes, lang='en'):
    ...
    if company.established_date:
        years_in_business = max(
            0, int((today - company.established_date).days / 365.25))

    total_requested_amount = sum(
        application.loan_amount or 0 for application in loan_applications)
    approved_applications = [
        application for application in loan_applications if application.application_status == 'approved']
    total_approved_amount = sum((application.approved_amount or application.loan_amount or 0)
                                for application in approved_applications)
    outstanding_balance = sum(
        application.outstanding_balance or 0 for application in loan_applications)
    overdue_accounts = sum(1 for application in loan_applications if (
        application.overdue_days or 0) > 0)

    accepted_bids = [bid for bid in project_bids if bid.status == 'accepted']
    open_disputes = [
        dispute for dispute in disputes if dispute.status == 'open']
    resolved_disputes = [
        dispute for dispute in disputes if dispute.status == 'resolved']
    training_coverage = company.safety_training_coverage
    osh_status = 'verified' if company.osh_safety_officer_verified and (
        training_coverage or 0) >= 80 else 'review'
    esg_status = 'verified' if (company.esg_policy_level or 'none') in [
        'basic', 'advanced'] else 'review'
    ...
    if latest_score:
        score_components = [
            {'label': score_labels.get('Financial strength', 'Financial strength'),
             'score': latest_score.financial_score or 0, 'max_score': 600},
            {'label': score_labels.get('Operational stability', 'Operational stability'),
             'score': latest_score.operational_score or 0, 'max_score': 250},
            {'label': score_labels.get('Qualifications', 'Qualifications'),
             'score': latest_score.qualification_score or 0, 'max_score': 200},
            {'label': score_labels.get('Customer reviews', 'Customer reviews'),
             'score': latest_score.customer_review_score or 0, 'max_score': 300},
        ]

    risk_factors = _build_monitoring_alerts(
        company, latest_score, overdue_accounts, open_disputes)
    if not risk_factors and not latest_score:
        risk_factors = [
            'No calculated score is available yet. Generate a score to unlock the full risk analysis.']
    ...
    recent_monitoring_activity.sort(key=lambda item: item['date'] or datetime.min.replace(
        tzinfo=timezone.utc), reverse=True)

    audit_snapshot = _build_audit_snapshot(
        company, loan_applications, disputes)
...
def _load_credit_report_context(company_id, lang='en'):
    ...
    credit_scores = CreditScore.query.filter_by(
        company_id=company_id).order_by(CreditScore.scored_at.desc()).all()
    latest_score = credit_scores[0] if credit_scores else None
    loan_applications = LoanApplication.query.filter_by(
        company_id=company_id).order_by(LoanApplication.applied_at.desc()).all()
    project_bids = ProjectBid.query.filter_by(
        company_id=company_id).order_by(ProjectBid.created_at.desc()).all()
    disputes = DisputeCase.query.filter_by(
        against_company_id=company_id).order_by(DisputeCase.opened_at.desc()).all()
    report = _build_credit_report(
        company, latest_score, credit_scores, loan_applications, project_bids, disputes, lang=lang)
...
def compare_reports():
    ...
    selected_company_ids = [int(value) for value in request.args.getlist(
        'company_ids') if value.isdigit()]
    ...
    if grade:
        candidate_companies = [company for company in candidate_companies if latest_scores.get(
            company.id) and latest_scores[company.id].credit_grade == grade]

    compare_reports = []
    for company_id in selected_company_ids[:3]:
        company, _, latest_score, report = _load_credit_report_context(
            company_id, lang=lang)
...
def add_company():
    ...
    if g.user.role == 'company_user' and not company_user_can_add(g.user):
        ...
        if lang == 'ch':
            ...
            flash(
                'You have already associated with a company and cannot add a new one.', 'warning')
            ...
            if 'documents' in request.files:
                ...
                if files and files[0].filename != '' and len(files) == len(document_types):
                    ...
                    for i, file in enumerate(files):
                        if file.filename:
                            expiry_date_str = request.form.getlist('expiry_dates')[i] if i < len(
                                request.form.getlist('expiry_dates')) else None
                            expiry_date = None
                            if expiry_date_str:
                                try:
                    ...
                    if files_data:
                        ...
                        if successful:
                            flash(
                                f'Company created successfully with {len(successful)} document(s) uploaded.', 'success')
                        else:
                            flash(
                                'Company created but document upload failed.', 'warning')

                        db.session.commit()
                        log_action('company_created', 'Company', company.id, {
                                   'company_name': company.company_name})
                        db.session.commit()
...
def view_company(id):
    ...
    credit_scores = CreditScore.query.filter_by(
        company_id=id).order_by(CreditScore.scored_at.desc()).all()
    latest_score = credit_scores[0] if credit_scores else None
...
def view_credit_report(id):
    ...
    company, credit_scores, latest_score, report = _load_credit_report_context(
        id, lang=lang)
    ...
    if request.method == 'POST':
        try:
            ...
            if 'documents' in request.files:
                ...
                if files and files[0].filename != '' and len(files) == len(document_types):
                    ...
                    if files_data:
                        ...
                        if failed:
                            error_messages = [
                                f"{f['file_name']}: {f['error']}" for f in failed]
                            flash(
                                f"Failed to upload {len(failed)} document(s): {'; '.join(error_messages)}", 'warning')

            db.session.commit()
            log_action('company_updated', 'Company', company.id,
                       {'company_name': company.company_name})
            db.session.commit()

            if documents_uploaded > 0:
                flash(
                    f'Company updated successfully with {documents_uploaded} new document(s).', 'success')
            else:
            ...
            if 'Request Entity Too Large' in error_msg or '413' in error_msg:
                flash(
                    'File size too large. Maximum allowed size is 50MB total. Please upload smaller files.', 'error')
            else:
...
def calculate_score(id):
    ...
    try:
        ...
        credit_score = scorer.save_score(
            company, result, notes=' Text Score Text ')
        db.session.add(credit_score)

        # Update trust_score_cached and risk_level in company table
        company.trust_score_cached = result['total_score']
        company.risk_level = result['risk_level']

        log_action('trust_score_calculated', 'Company',
                   company.id, {'score': result['total_score']})
        db.session.commit()

        flash(
            f'Trust score calculated: {result["total_score"]} ({result["credit_grade"]})', 'success')
...
def delete_company(id):
    ...
    try:
        log_action('company_deleted', 'Company', company.id,
                   {'company_name': company.company_name})
...
def upload_documents(id):
    ...
    company = get_or_404(Company, id)
    require_ownership(can_manage_company(g.user, company) or g.user.role ==
                      'company_user' and g.user.company_id == company.id)
    ...
    if len(files) != len(document_types):
        ...
        flash(
            f'Number of files ({len(files)}) must match number of document types ({len(document_types)}).', 'error')
        return redirect(url_for('companies.view_company', id=id))
    ...
    for i, file in enumerate(files):
        ...
        print(
            f"      描述: {descriptions[i] if i < len(descriptions) else 'None'}")

        expiry_date = None
        if i < len(expiry_dates) and expiry_dates[i]:
            try:
                expiry_date = datetime.strptime(
                    expiry_dates[i], '%Y-%m-%d').date()
    ...
    try:
        ...
        if successful:
            ...
            flash(
                f'Successfully uploaded {len(successful)} document(s).', 'success')
